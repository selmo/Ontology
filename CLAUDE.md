...
**v3.13 변경점 (공통표준용어 연계):**
- ✅ **용어 대폭 확장**: 221개 → 440개 (+219개, +99%)
- ✅ **공통표준용어 연계**: 9,027개 중 219개 선별 추가
- ✅ **용어 커버리지 향상**: 63.3% → 81.6% (+18.3%p)
- ✅ **STD-STDTERM 자동 매핑**: 219개 (EXACT_MATCH, confidence 1.0)
- ✅ **ID 생성 로직 강화**: 하위 용어(children) 재귀 체크
- ✅ **검증 스크립트 버그 수정**: 이스케이프 따옴표 처리 개선
- ✅ **도메인별 균형 추가**: 번호, 코드, 명, 일자 등 핵심 항목 우선
...
- **v3.13** (2025-12-11): **공통표준용어 연계** - 221개 → 440개 용어 (+219개, +99%), 용어 커버리지 81.6%

## 주요 성과 (v2.1 → v3.13)

- **분류:** 175 → 376개 (+115%)
- **용어:** 124 → **440개 (+255%, +99%는 v3.13에서)**
...
4. ~~**공통표준용어 연계**: 9,027개 표준용어 → 219개 선별 추가~~ ✅ 완료 (v3.13: 81.6% 커버리지)
5. **Close Match 수동 검토**: 77개 후보 (confidence 0.7-0.9) → 20~30개 추가 예상
6. **SKOS RDF 출력 구현**: 국제 표준 호환 (RDF/Turtle/JSON-LD)
7. **디지털커머스·과학기술 도메인 표준 매핑 강화**: e-commerce, scientific taxonomy 추가 매핑
